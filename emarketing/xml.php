...
switch ($handlerMethod) {
	...
	case 'editexistinguser':
		$editMode = ($requestMethod == 'editexistinguser');
		...
		if ($editMode) {
			$status = $user->Load($function_params['userid'], true);
			if (!$status) {
		...
		// ----- Check if username is available to be used
			$existingUser = $user->Find($function_params['username']);
