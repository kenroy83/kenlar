...
class Jobs_Autoresponders_API extends Jobs_API
{
	...
	*/
	function ProcessJob($queueid=0)
	{
		...
		IEM::sessionSet('__CurrentUser_ID', $this->user->userid);
		IEM::sessionSet('__CurrentUser_Object', $this->user);
		...
		while ($recipients = $this->FetchFromQueue($queueid, 'autoresponder', 1, 500, $this->autoresponder_api->Get('hoursaftersubscription'))) {
			...
			foreach ($recipients as $p => $recipientid) {
				...
				*/
				...
				// ----- Record credit usage every 100 emails sent
					if ($credit_available !== true) {
						...
						if ($credit_used >= 100) {
							...
							if (!$status) {
								$this->_log("Cannot record usage -- userid = {$ownerid}, credit_used = {$credit_used}, job_id = {$jobid}, stat_id = {$this->statid}");
								return false;
							}
