	...
	*/
	function MarkNewsletterFinished($statids=array(), $original_queuesizes=array())
	{
		...
		if ($original_queuesize > $total_recipients) {
			...
			$query = "UPDATE [|PREFIX|]users SET maxemails = maxemails + {$credits} WHERE userid={$userid} AND unlimitedmaxemails='0'";
			$this->Db->Query($query);
		} else {
			/**
			...
			if ($difference > 0) {
				$query = "UPDATE [|PREFIX|]users SET maxemails = maxemails - {$difference} WHERE userid={$userid} AND unlimitedmaxemails='0'";
				$this->Db->Query($query);
			}
		}
	...
	function RefundFixedCredit($jobid)
	{
		...
		$result = $this->Db->Query($query);
		if (!$result) {
			trigger_error(__CLASS__ . '::' . __METHOD__ . ' - Cannot update user record', E_USER_NOTICE);
			return false;
		}
	...
	*/
	function ReCheckUserStats(&$user_object, $original_queuesize=0, $new_queuesize=0, $queuetime=0, $statid=0)
	{
		...
		*/
		...
		$existing_queuesize = API_USERS::creditAvailableThisMonth($user_object->GetNewAPI());
	...
	function SummarizeEmailSend($queueid=0, $statid=0, $userid=0)
	{
		...
		// ----- Calculate and insert statistic to user_stats_emailsperhour table
			if ($totalRecipient > 0) {
				...
				$tempCurrentTime = $tempHours * 3600;
