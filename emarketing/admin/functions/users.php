...
class Users extends SendStudio_Functions
{
		...
		switch ($action) {
			...
			case 'save':
					...
					if (!$error) {
						 ...
						 */
							$areaMapping = array(
								...
								'forcedoubleoptin' => 'forcedoubleoptin',
								'forcespamcheck' => 'forcespamcheck',
								'infotips' => 'infotips',
				...
				if ($userid == $thisuser->userid) {
					...
					IEM::sessionSet('__CurrentUser_ID', $new_user->userid);
					IEM::sessionSet('__CurrentUser_Object', $new_user);
				}
			...
			case 'create':
				...
				if (!$user->Find($_POST['username'])) {
					foreach (array('username', 'fullname', 'emailaddress', 'status', 'admintype', 'editownsettings', 'listadmintype', 'segmentadmintype', 'usertimezone', 'textfooter', 'htmlfooter', 'templateadmintype', 'unlimitedmaxemails', 'forcedoubleoptin', 'forcespamcheck', 'infotips', 'smtpserver', 'smtpusername', 'smtpport', 'usewysiwyg', 'enableactivitylog', 'xmlapi', 'xmltoken','googlecalendarusername','googlecalendarpassword') as $p => $area) {
	...
	*/
	function PrintManageUsers()
	{
		...
		$recordTotal = $api->GetUsers(0, $pageSortInfo, true);
		$records = $api->GetUsers(0, $pageSortInfo, false, $start, $pageRecordPP);

		for ($i = 0, $j = count($records); $i < $j; ++$i) {
			$records[$i]['processed_CreateDate'] = $this->PrintDate($records[$i]['createdate']);
			$records[$i]['processed_LastLoggedIn'] = $this->PrintDate($records[$i]['lastloggedin']);
		}
