	...
	*/
					...
					<table class="Panel" border="0" cellpadding="2" cellspacing="0" width="100%">
						...
						<tr>
							...
							<td style="padding:10px">
								...
								<?php
									if (SENDSTUDIO_SAFE_MODE) {
										echo '<br/><br/><b>There were problems copying images and attachments to their new locations. This is because safe-mode is enabled on your server. This step has been bypassed, please contact help@sCRiPTz-TEAM.iNFO once your upgrade has finished.</b><br/>';
									} else {
	...
	function ShowStep_4()
	{
		...
		if (!empty($upgrade_errors)) {
			?>
				<table cellspacing="0" cellpadding="0" width="95%" align="center">
					...
					<tr>
						<td class="Text">
							<br/>There were problems upgrading your database. Please contact help@sCRiPTz-TEAM.iNFO and tell them the following errors occurred:<br/><br/>
		...
		if (!class_exists('MySQLDb', false)) {
			require(IEM_PATH . '/ext/database/mysql.php');
		}
		$db_type = 'MySQLDb';
		$db = new $db_type();
	...
	function UpgradeDatabase()
	{
		...
		if (!class_exists(SENDSTUDIO_DATABASE_TYPE . 'Db', false)) {
			require(IEM_PATH . '/ext/database/' . SENDSTUDIO_DATABASE_TYPE . '.php');
		}
