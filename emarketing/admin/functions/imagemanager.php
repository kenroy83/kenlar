...
* @author 		David <david.chandra@sCRiPTz-TEAM.iNFO>
*
* @package SendStudio
...
class ImageManager extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
		...
		$type = (isset($_GET['Type'])) ? strtolower(urldecode($_GET['Type'])) : '';
		$typeId = (isset($_GET['TypeId'])) ? strtolower(urldecode($_GET['TypeId'])) : 0;
	...
	*/
	function RemoteUpload($type, $typeId) {
		...
		if ($_FILES['Filedata']['error'] == UPLOAD_ERR_OK) {
			...
			}elseif(file_exists($api->GetImagePath() . '/' . $name)){
				$_FILES['Filedata']['duplicate'] = true;

			}else{
				$_FILES['Filedata']['duplicate'] = false;
				move_uploaded_file($tmp_name, $api->GetImagePath() . '/' . $name);
			}
		}
