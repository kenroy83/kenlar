...
var gcAllDay = <?php if(isset($GLOBALS['GoogleCalendarAllDay'])) print $GLOBALS['GoogleCalendarAllDay']; ?>;
	...
	function googleCalendarDates(field) {
		...
		if (!allday) {
			if (!(reTimeFrom = $('#gcTimeFrom').val().match(/(\d+):(\d+)\s*(am|pm)/i))) {
				//alert("<?php print GetLang('EnterValidDate'); ?>");
				//return false;
			}
	...
	function googleCalendarForm(fieldid,fromform,dataobj) {
		if (dataobj == null) {
			dataobj = {
				what: '<?php print GetLang('FollowUp'); ?>',
				datefrom: '',
				timefrom: '',
				dateto: '',
				timeto: '',
				allday: gcAllDay,
				where: '',
				description: "<?php print GetLang('GoogleCalendarDescription'); ?>\n\n<?php if(isset($GLOBALS['GoogleCalendarLink'])) print $GLOBALS['GoogleCalendarLink']; ?>"
			}
		}
		if (fieldid) {
			if (fromform) {
	...
	function googleCalendarSubmit() {
		if ($('#gcWhat').val() == '') {
			alert('<?php print GetLang('GoogleCalendarEnterDescription'); ?>');
...
<table class="Text" style="padding:10px;width: 100%;">
	<tr>
		<td colspan="2" style="padding-bottom:5px;">
			<?php if(isset($GLOBALS['GoogleCalendarIntroText'])) print $GLOBALS['GoogleCalendarIntroText']; ?>
		</td>
	</tr>
	<tr>
		<td width="5" nowrap="nowrap"><?php print GetLang('What'); ?>:</td>
		<td><input type="text" id="gcWhat" class="Field" style="width: 100%;"  name="google[what]"></td>
	</tr>
	<tr>
		<td><?php print GetLang('When'); ?>:</td>
		<td>
			<span id="gcDates">
			...
			<label for="gcAllDay">
				<?php print GetLang('AllDay'); ?>
			</label>
		</td>
	</tr>
	<tr>
		<td><?php print GetLang('Where'); ?>:</td>
		<td><input type="text" style="width: 100%;" class="Field" id="gcWhere" name="google[where]"></td>
	</tr>
	<tr>
		<td valign="top"><?php print GetLang('Description'); ?>:</td>
		<td><textarea id="gcDescription" style="width: 100%; height: 100px;" class="Field" name="google[description]"></textarea></td>
	</tr>
	<tr>
		<td></td>
		<td>
			<button class="FormButton" id="gcSave" onclick="googleCalendarSubmit(); return false;"><?php print GetLang('Save'); ?></button>
			<button class="FormButton" id="gcCancel" onclick="top.tb_remove(); return false;"><?php print GetLang('Cancel'); ?></button>
