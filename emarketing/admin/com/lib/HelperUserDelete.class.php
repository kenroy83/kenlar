...
class HelperUserDelete
{
	...
	static private function _deleteUser($userid)
	{
		...
		// ----- Delete "easy" data (ie. data that can be deleted without processing anything else)
			...
			if (($temp = self::_deleteFromSimpleTable('user_access', 'userid', $userid)) === false) return $status; else $status['data']['user_access'] = $temp;
			if (($temp = self::_deleteFromSimpleTable('user_permissions', 'userid', $userid)) === false) return $status; else $status['data']['user_permissions'] = $temp;
