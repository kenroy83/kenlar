...
var AdminImageManager = {
	...
	GetImageRow: function() {
		return '	<span class="ManageImageBox" id="%%image_id%%" >		<input type="checkbox" id="deleteimages[]" value="%%image_realname%%" class="%%image_id%%_Class" /><input class="TemplateHeading inPlaceImageBoxDefault" id="%%image_id%%_name" value="%%image_name%%" />				<input type="hidden" id="%%image_id%%_realname" value="%%image_realname%%" />		<br />		<a href=\'%%image_url%%\' id="%%image_id%%_url" target="_blank">		<div style="width: 200px; height: 150px; margin-top: 5px;">			<img src=\'%%image_url%%\' style=" border: solid 1px #CACACA;"  id="%%image_id%%_image" width="%%image_width%%" height="%%image_height%%" title="Click here to view the full size image" /><br />		</div>			<img width="10" hspace="3" height="11" border="0" src="images/magnify.gif" />%%LNG_ImageManagerViewFullSize%%</a><br />		Size: %%image_size%% <br />		Dimensions: %%image_dimensions%%px<br />		<input type="button"  class="Field150" id="%%image_id%%_delete" value="%%LNG_DelThisImage%%" />	</span>';
	},
		...
		$.ajax({
			...
			success: function(resp){
				...
				if (hasReqestedFlashVersion) {
					swfu = new SWFUpload({
						// Backend Settings
						upload_url: "%%GLOBAL_adminUrl%%/index.php?Page=ImageManager&Action=remoteupload",
						// File Upload Settings
						file_size_limit : "20 MB",
						file_types : "*.jpg;*.gif;*.png;*.tiff,*.bmp,*.jpeg",
						file_types_description : " Images",
						file_upload_limit : "0",
						post_params: {"{$SessionName}": "{$SessionID}", "what": "imagemanagerupload"},
						...
						// Flash Settings
						flash_url : "%%GLOBAL_adminUrl%%/images/swfupload.swf",
