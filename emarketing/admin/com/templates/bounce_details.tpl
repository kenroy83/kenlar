...
{capture name=nodejoin}{if $IEM.CurrentPage == 'bounce'}<img src="images/nodejoin.gif" />{/if}{/capture}
{capture name=blankimg}{if $IEM.CurrentPage == 'bounce'}<img width="20" height="20" src="images/blank.gif" />{/if}{/capture}

{if $IEM.CurrentPage != 'bounce' && $IEM.CurrentPage != 'settings'}
<tr>
			...
			Application.Page.BounceInfo = {
				...
				SetupClickEvents: function() {
					...
					f.cmdTestBounce && $(f.cmdTestBounce).click(function() {
						...
						var url = 'index.php?Page=Bounce&Action=PopupBounceTest{if $IEM.CurrentPage != 'bounce'}&InPlace=true{/if}' + Application.Page.BounceInfo.getBounceParameters() + '&keepThis=true&TB_iframe=true&height=240&width=400&modal=true&random=' + new Date().getTime();
						tb_show('', url, '');
						return true;
					});
...
<tr style="display: %%GLOBAL_ShowBounceInfo%%" class="YesProcessBounce">
	...
	<td style="{$style}">
		{$nodejoin} <input type="text" name="bounce_server" id="bounce_server" class="Field250 form_text bounceSettings" value="%%GLOBAL_Bounce_Server%%">&nbsp;{if $IEM.CurrentPage == 'settings'}%%LNG_HLP_DefaultBounceServer%%{else}%%LNG_HLP_ListBounceServer%%{/if}
	</td>
</tr>
<tr style="display: %%GLOBAL_ShowBounceInfo%%" class="YesProcessBounce">
	<td class="FieldLabel">
	...
	<td style="{$style}">
		{$blankimg} <input type="text" name="bounce_username" class="Field250 form_text bounceSettings" value="%%GLOBAL_Bounce_Username%%">&nbsp;{if $IEM.CurrentPage == 'settings'}%%LNG_HLP_DefaultBounceUsername%%{else}%%LNG_HLP_ListBounceUsername%%{/if}
	</td>
</tr>
<tr style="display: %%GLOBAL_ShowBounceInfo%%" class="YesProcessBounce">
	<td class="FieldLabel">
	...
	<td style="{$style}">
		{$blankimg} <input type="password" name="bounce_password" class="Field250 form_password bounceSettings" value="%%GLOBAL_Bounce_Password%%" autocomplete="off" />&nbsp;{if $IEM.CurrentPage == 'settings'}%%LNG_HLP_DefaultBouncePassword%%{else}%%LNG_HLP_ListBouncePassword%%{/if}
	</td>
</tr>
<tr style="display: %%GLOBAL_ShowBounceInfo%%" class="YesProcessBounce">
	<td class="FieldLabel">
