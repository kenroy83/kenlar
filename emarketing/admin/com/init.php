...
function __autoload($classname)
{
	...
	// ----- Work out the full file name and class name
		...
		$file = dirname(__FILE__) . '/lib/' . $subdirectory . $filePart . '.class.php';
...
// --------------------------------------------------------------------------------
	...
	// ----- Set error and exception handler
		//set_error_handler() // TODO: set error handler, and deprecate the existing error handler
	...
	// --------------------------------------------------------------------------------
		if (!defined('IEM_NO_CONTROLLER') || constant('IEM_NO_CONTROLLER') !== true) {
			...
			// --------------------------------------------------------------------------------
				if (!IEM::getCurrentUser()) {
					...
					while (true) {
						...
						IEM::sessionSet('__CurrentUser_ID', $tempUser->userid);
						IEM::sessionSet('__CurrentUser_Object', $tempUser);
						$tempUser->UpdateLoginTime();
						$tempValid = true;

						// Check if he have login preferences
			...
			// This is the new page structure
				...
				if (!is_callable(array($tempPageObject, $tempAction), true)) {
