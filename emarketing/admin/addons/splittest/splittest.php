...
class Addons_splittest extends Interspire_Addons
{
	...
	static function RegisterAddonPermissions()
	{
		...
		$perms = array (
			'splittest' => array (
				'addon_description' => $description['description'],
	...
	private function _ShowForm($splitid=null, $chosen_campaigns=array())
	{
		...
		require(SENDSTUDIO_API_DIRECTORY . '/newsletters.php');
	...
	public function Admin_Action_Send()
	{
		...
		require dirname(__FILE__) . '/splittest_send.php';
	...
	*/
	...
	public static function CleanupPartialSends(EventData_IEM_SENDSTUDIOFUNCTIONS_CLEANUPOLDQUEUES $data)
	{
		...
		require dirname(__FILE__) . '/api/splittest_send.php';
		$send_api = new Splittest_Send_API;

		$send_size = 0;
		if (isset($send_details['SendSize'])) {
		...
		if ($send_size > 0) {
			...
			if (!$unlimited_emails) {
				...
				$user->Save(false);
				// ----- TODO think of a better way... The session variable "__CurrentUser_Object" should only be modified in one place
					IEM::sessionSet('__CurrentUser_Object', $user);
				// -----
			}
		}
		...
		if (isset($send_details['Stats'])) {
			if (!class_exists('Stats_API', false)) {
				require SENDSTUDIO_API_DIRECTORY . '/stats.php';
			}
	...
	public static function DeleteSchedules(EventData_IEM_SCHEDULE_DELETEJOBS $data)
	{
		...
		if ($reload_user) {
			...
			IEM::sessionSet('__CurrentUser_ID', $myuser->userid);
			IEM::sessionSet('__CurrentUser_Object', $myuser);
		}
